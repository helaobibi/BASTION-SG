-- 创建模块
local BeastMasterModule = Bastion.Module:New('BeastMasterModule')
...
local ArcaneShot = SpellBook:GetSpell(49045)              -- 奥术射击
...
local PorcupineBait = SpellBook:GetSpell(1284198)         -- 豪猪诱饵
local EagleShot = SpellBook:GetSpell(1284199)             -- 雄鹰射击
local EagleShotBuff = SpellBook:GetSpell(1284379)         -- 雄鹰射击BUFF
...
local BestTarget = Bastion.UnitManager:CreateCustomUnit('besttarget', function()
    ...
    Bastion.ObjectManager.enemies:each(function(unit)
        ...
        -- 4. 目标距离玩家至少5码
        -- 5. 玩家面向该目标
        if unit:IsAffectingCombat() and SteadyShot:IsInRange(unit)
...
local HERUI = {
    ...
    PorcupineBaitDistance = createUIAccessor("PorcupineBaitDistance", 10),  -- 新增：豪猪诱饵距离，默认10码
...
PetControlAPL:AddAction("PetFollow", function()
    local petTargetName = PetTarget:Exists() and PetTarget:GetName() or ""
    if Pet:Exists() and Pet:IsAlive()
...
-- 豪猪诱饵（只在陷阱CD中释放，避免冲突）
AoEAPL:AddSpell(
    PorcupineBait:CastableIf(function(self)
        return Target:Exists()
            and self:IsKnownAndUsable()
            and not Player:IsChanneling()
            and Target:IsAlive()
            and Target:IsEnemy()
            and (TrapSpell:GetCooldownRemaining() > 1.1 or HERUI.ExplosiveTrap() == false)  -- 陷阱CD中或陷阱关闭时才释放
    end):SetTarget(Target):PreCast(function(self)
        -- 检查是否正在读条稳固射击，如果是则停止施法
        if Player:IsCastingOrChanneling() and Player:GetCastingOrChannelingSpell():GetID() == 49052 then
            SpellStopCasting()
        ...
        -- 参数：半径8码，范围35码，最少需要2个敌人才使用质心定位
        local centroid = Bastion.UnitManager:FindEnemiesCentroid(8, 35, 2)

        if centroid then
            -- 使用质心位置，往玩家方向来指定距离
            local direction = (playerPos - centroid):Normalize()
            position = centroid + direction * HERUI.PorcupineBaitDistance()
        else
            -- 使用目标位置，往玩家方向来指定距离
            local targetPos = Target:GetPosition()
            local direction = (playerPos - targetPos):Normalize()
            position = targetPos + direction * HERUI.PorcupineBaitDistance()
        end
...
AoEAPL:AddSpell(
    MultiShotSpell:CastableIf(function(self)
        return Target:Exists()
            ...
            and TrapSpell:GetCooldownRemaining() > 1.1  -- 陷阱CD中才释放
            and PorcupineBait:GetCooldownRemaining() > 1.1  -- 豪猪诱饵CD中才释放
    end):SetTarget(Target):PreCast(function(self)
        -- 检查是否正在读条稳固射击，如果是则停止施法
        if Player:IsCastingOrChanneling() and Player:GetCastingOrChannelingSpell():GetID() == 49052 then
            SpellStopCasting()
...
-- 雄鹰射击
DefaultAPL:AddSpell(
    EagleShot:CastableIf(function(self)
        return Target:Exists()
            and Target:IsAlive()
            and self:IsKnownAndUsable()
            and Player:GetAuras():FindMy(EagleShotBuff):GetCount() < 2
    end):SetTarget(Target)
)
...
DefaultAPL:AddSpell(
    PorcupineBait:CastableIf(function(self)
        ...
        -- 从目标位置往玩家方向退指定距离
        local direction = (playerPos - targetPos):Normalize()
        local position = targetPos + direction * HERUI.PorcupineBaitDistance()
...
DefaultAPL:AddSpell(
    SteadyShot:CastableIf(function(self)
        return (TrapSpell:GetCooldownRemaining() > 0.5 or HERUI.ExplosiveTrap() == false)
            and Target:Exists()
            and Target:IsAlive()
            and self:IsKnownAndUsable()
            and (HERUI.ViperStingMode() or Target:GetAuras():FindMy(Serpent):GetRemainingTime() > 3)  -- 蝰蛇钉刺模式时跳过检查
    end):SetTarget(Target)
)
...
-- ===================== 模块同步 =====================
BeastMasterModule:Sync(function()
...
-- ===================== 注册模块 =====================
Bastion:Register(BeastMasterModule)
